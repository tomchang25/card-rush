...
func get_first_conditional_action() -> EnemyAction:
    for action: EnemyAction in get_children():
        if not action or action.type != EnemyAction.Type.CONDITION_BASED:
            continue
...
func get_chance_based_action() -> EnemyAction:
    ...
    for action: EnemyAction in get_children():
        ...
        current_weight_sum += action.chance_weight

        if current_weight_sum >= roll:
            return action
...
func calculate_total_weight() -> void:
    total_weight = 0.0 # Reset the total weight
    for action: EnemyAction in get_children():
        if not action or action.type != EnemyAction.Type.CHANCE_BASED:
            continue
